class Ball2 {
    ...
    initialize() {
        ...
        // Chỉ tạo graphics, không tạo physics body
        this.graphics = this.createGraphics();
        
        // Container chỉ chứa graphics
	...
	applyInterpolatedState(state) {  // IS IT NEEDED TO CALCULATE THE VELOCITY 
        if (this.container) {
            const lerpFactor = 0.3; // Điều chỉnh độ mượt (0.1 to 0.5)
            
            // Lerp để di chuyển mượt hơn
            this.visualPosition.x += (state.position.x - this.visualPosition.x) * lerpFactor;
            this.visualPosition.y += (state.position.y - this.visualPosition.y) * lerpFactor;
            
            this.container.setPosition(this.visualPosition.x, this.visualPosition.y);
        }
    }
    ...
    update() {
		this.updateInterpolation(Date.now());
    }
