class Ball {
    ...
    printStoredCurrentVelocityDiff() {
        const oldVel = this.oldVelocities.get(this.body.id);
        if (!oldVel) return;
    
        const currentVel = this.getVelocity();
        
        const oldMagnitude = Math.sqrt(oldVel.x * oldVel.x + oldVel.y * oldVel.y);
        const currentMagnitude = Math.sqrt(currentVel.x * currentVel.x + currentVel.y * currentVel.y);
        const magnitudeDiff = Math.abs(oldMagnitude - currentMagnitude);
        
        console.log("=== Stored vs Current Velocity Analysis ===");
        console.log("Stored oldVelocity:", oldVel);
        console.log("Current velocity:", currentVel);
        console.log("Stored magnitude:", oldMagnitude.toFixed(2));
        console.log("Current magnitude:", currentMagnitude.toFixed(2));
        console.log("Magnitude difference:", magnitudeDiff.toFixed(2));
        console.log("===============================");
